...
do_ecdh_evp(int nid, const char *text)
{
	...
	if (nid == NID_X25519) {
		pctx1 = EVP_PKEY_CTX_new_id(NID_X25519, NULL);
	} else  {
	...
	/* Generate key */
	if (nid == NID_X25519) {
		if (EVP_PKEY_keygen_init(pctx1) <= 0)
			goto err;
		if (EVP_PKEY_keygen(pctx1, &pkey1) <= 0) {
	...
	/* Create the context for parameter generation */
	if (nid == NID_X25519) {
		pctx2 = EVP_PKEY_CTX_new_id(NID_X25519, NULL);
	} else {
	...
	/* Generate key */
	if (nid == NID_X25519) {
		if (EVP_PKEY_keygen_init(pctx2) <= 0)
			goto err;
		if (EVP_PKEY_keygen(pctx2, &pkey2) <= 0) {
			fprintf(stderr, "EVP_PKEY_keygen error.\n");
			goto err;
		}
	} else {
